use frameworks::cuda::sh::cudaError_enum;
...
impl From<cudaError_enum> for ErrorKind {

    fn from(cuda_error_enum: cudaError_enum) -> ErrorKind {
        use frameworks::cuda::sh::cudaError_enum::*;
        use self::ErrorKind::*;
...
impl ErrorKind {
    ...
    fn as_str(&self) -> &'static str {
        ...
        match *self {
            InvalidValue => "one or more of the parameters passed to the API call is not within an \
            acceptable range of values",
            OutOfMemory => "unable to allocate enough memory to perform the requested operation",
            NotInitialized => "either the CUDA driver has not been initialized or the \
            initialization has failed",
            ...
            InvalidDevice => "the device ordinal supplied by the user does not correspond to a \
            valid CUDA device",
            ...
            UnsupportedLimit => "the CUlimit passed to the API call is not supported by the \
            active device",
            ContextAlreadyInUse => "the CUcontext passed to the API call can only be bound to a \
            single CPU thread at a time but is already bound to a CPU thread",
            ...
            IllegalAddress => "while executing a kernel, the device encountered a load or store \
            instruction on an invalid memory address",
            LaunchOutOfResources => "launch did not occur because it did not have appropriate \
            resources",
            ...
            PeerAccessAlreadyEnabled => "a call to cuCtxEnablePeerAccess() is trying to re-enable \
            peer access to a context which has already had peer access to it enabled",
            PeerAccessNotEnabled => "cuCtxDisablePeerAccess() is trying to disable peer access \
            which has not been enabled yet via cuCtxEnablePeerAccess()",
            PrimaryContextActive => "the primary context for the specified device has already \
            been initialized",
            ContextIsDestroyed => "calling thread context has been destroyed or is a primary \
            context which has not yet been initialized",
            Assert => "a device-side assert triggered during kernel execution",
            TooManyPeers => "the hardware resources required to enable peer access have been \
            exhausted for one or more of the devices passed to cuCtxEnablePeerAccess()",
            HostMemoryAlreadyRegistered => "the memory range passed to cuMemHostRegister() has \
            already been registered",
            HostMemoryNotRegistered => "the pointer passed to cuMemHostUnregister() does not \
            ...
            IllegalInstruction => "while executing a kernel, the device encountered an \
            illegal instruction",
            MisalignedAddress => "misaligned address",
            InvalidAddressSpace => "invalid address space",
            InvalidPc => "while executing a kernel, the device program counter wrapped its \
            address space",
            ...
            NotSupported => "the attempted operation is not supported on the current system \
            or device",
...
impl Error {
    ...
    /// Creates a new error from a known kind of error as well as an arbitrary `inner` error.
    ...
    pub fn get_ref(&self) -> Option<&(error::Error + Send + Sync + 'static)> {
        ...
        match self.inner {
            Some(ref inner) => Some(inner.deref()),
            _ => None
        }
    }
